...
'''
    Interpolation script for a trained Property - Variational Autoencoder (VAE) model.
    This script includes:
    - Loading the model and latent vectors
    - Need a target CSV file with random target values
...
z_csv_path = os.path.join(project_root, "latent_vectors", "uniRandint_z_3D100.csv")
properties_path = os.path.join(project_root, "data", "data3D100.csv")
output_csv_path = os.path.join(project_root, "inverse_results", "uniRandint_optimal_z_progress.csv")
latent_dim = 250
epochs = 2000
lr = 0.005
...
# Load target values
random_targets = pd.read_csv(target_csv_path)
...
def optimize_latent_vector(target_nF, target_perm_x):
    target = torch.tensor([target_nF, target_perm_x], dtype=torch.float32).to(device)
...
for idx, row in random_targets.iterrows():
    target_nF = row["Target n_F"]
    target_perm_x = row["Target Permeability_x"]
    target_perm_y = row["Target Permeability_y"]
    target_perm_z = row["Target Permeability_z"]

    print(f"\nOptimizing for Case {idx + 1}...")
    z_star = optimize_latent_vector(target_nF, target_perm_x, target_perm_y, target_perm_z)

    row_data = [idx + 1, target_nF, target_perm_x, target_perm_y, target_perm_z, *z_star.flatten()]
    ...
    print(f"Saved optimal latent vector for Case {idx + 1}.")
